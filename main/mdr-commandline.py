...
def process_wrapper(file_args):
    """
    Wrapper function for use with multiprocessing
    """

    pdb_dir, file, resfile_dir, mutation_dictionary, minimize = file_args
    print(f'Processing {file}', flush=True)
    process_frame(pdb_dir, file, resfile_dir, mutation_dictionary, minimize)


def process_ensemble(pdb_dir, resfile_dir, mutation_dictionary, minimize):
    """
    ...
    frames_left_to_process = [f for f in files
                              if not path.exists(
                                f"{int(path.basename(f).replace('frame', '').replace('.pdb', '')):04}.npz"
                                )]

    num_workers = min(cpu_count(), len(frames_left_to_process))

    with Pool(num_workers) as pool:
        pool.map(process_wrapper, [(pdb_dir, file, resfile_dir, mutation_dictionary, minimize) for file in frames_left_to_process])
...
def main(pirate_noises):
    ...
    process_ensemble(pdb_house, resfile_house, ssm_mutations, minimize)
