...
This file is the for function calls in a jupyter notebook.
Basically only exists so that I can import it into a
jupyter notebook.
...
def assign_rounds(sorts):
    ...
    while (missing):
        ...
        if (iterr > 100):
            sys.exit(
                "Could not assign rounds. Parent structure does not make sense. All pools must be derived from expression"
                + " (With the exception of a naive pool)")


def load_pose_data(pdbs, scorefxn):
...
def my_dg_fold_fit(xs, y_lb, y_ub, ci_width_avg_dev=0.25, dg_fold_lower_clip=-15, dg_fold_upper_clip=15):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pooled_counts", type=str, help="Same as estimate_affinity_from_ngs.py")
    parser.add_argument("--sorts", type=str, help="Same as estimate_affinity_from_ngs.py")
    parser.add_argument("--affinity_estimate_out", type=str,
                        help="The output from estimate_affinity_from_ngs.py. Your SSM mutations must be labeled"
                             ...
                             + " for examples.")
    parser.add_argument("--fast_cart_fastrelax_out", type=str,
                        help="The scorefile from your fast_cart_fastrelax trajectories. The file should"
    ...
    parser.add_argument("--parent_pdbs", type=str, nargs="+",
                        help="The pdb files you wish to perform this analysis on. Name must coordinate with"
                             + " affinity_estimate_out and fast_cart_fastrelax_out.")

    parser.add_argument("--random_copies", type=int, default="100",
                        help="How many shuffled datasets should be produced. More is always better"
                             + " but is slower. Fewer leads to more noise.")
    parser.add_argument("--rosetta_kcal_buffer", type=float, default=1,
                        help="How close does the Rosetta estimation need to get to the experimental"
    ...
    parser.add_argument("--dont_estimate_dg_fold", action="store_true",
                        help="Skip the dg_fold estimation step. Although this step makes sense"
    ...
    parser.add_argument("--dg_fold_ci_width_avg_dev", type=float, default=0.25,
                        help="dg_fold is fitted with least-squares regression. The final"
    ...
    parser.add_argument("--entropy_pool_conc_factor", type=float, default=0.1,
                        help="Which pool should we use for sequence entropy?"
    ...
    parser.add_argument("--print_shuffle_copy_info", action="store_true",
                        help="Print the calculated values for all of the random_copies too.")
    parser.add_argument("--dump_correlation_data", action="store_true",
                        help="Dump all the data that goes into these calculations. Helpful if you "
                             + " want to make a graph of predicted vs experimental energy for instance.")
    parser.add_argument("--dump_shuffled_data", action="store_true",
                        help="Used with --dump_correlation_data. Use this to dump all the random copies too.")
    ...
    kd_df = kd_df[
        ['kd_lb', 'kd_ub', 'lowest_conc', 'highest_conc', 'low_conf', 'avid_doesnt_agree', 'description']].copy()
    ...
    score_df = score_df[
        ['ddg_no_repack', 'total_score_monomer', 'total_score_target', 'total_score', 'description']].copy()
    ...
    sorts = pd.read_csv(args.sorts, sep=",")
    counts = pd.read_csv(args.pooled_counts, sep=r"\s+")
    ...
    sequences, pose_df = load_pose_data(args.parent_pdbs, scorefxn)
    ...
    to_print = "We have %i datapoints to consider. %i datapoints with scores. And %%i with both." % (
    len(kd_df), len(score_df))
    ...
    if ((~kd_df['ddg_no_repack'].isnull() & ~kd_df['native']).sum() == 0):
        print("")
        print(
            "Error! None of your scorefile names matched your KD names. The names must match nearly exactly. This script will trim"
    ...
    for parent, subdf in kd_df.groupby('ssm_parent'):
        ...
        print("    %5i / %5i (%2i%%)  --   %5i / %5i (%2i%%) -- %s" % (
        has_score, expected_number, has_score / expected_number * 100,
    ...
    for parent, subdf in kd_df.groupby('ssm_parent'):
        ...
        if (len(natives) == 0):
            ...
            continue
        parent_kd = np.clip(np.sqrt(natives['kd_ub'].mean() * natives['kd_lb'].mean()),
                            natives['lowest_conc'].iloc[0] / 10,
    ...
    to_merge = df_vs_native[
        ['delta_ddg_no_repack', 'delta_total_score_monomer', 'delta_total_score_target', 'delta_total_score',
    ...
    if (not args.dont_estimate_dg_fold):
        ...
        for name, subdf in df.groupby('ssm_parent'):
            ...
            subdf = subdf[((subdf['is_monomer_core'] | subdf['is_monomer_boundary'] | subdf['is_monomer_surface']))
                          ...
                          & ((subdf['parent_letter'] != 'P') & (subdf['parent_letter'] != 'C') & (
                        subdf['parent_letter'] != 'G'))
                          & ((~subdf['is_loop']))
    ...
    use_df = use_df[
        ['ssm_parent', 'dg_fold', 'dg_fold_lb', 'dg_fold_ub', 'delta_total_score_monomer', 'delta_total_score_target',
